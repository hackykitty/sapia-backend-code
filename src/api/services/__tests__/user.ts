import faker from 'faker'

import db from '@src/utils/db'
import { createDummy, createDummyAndAuthorize } from '@src/tests/user'
import user from '../user'

beforeAll(async () => {
  await db.open()
})

afterAll(async () => {
  await db.close()
})

it('login perfromance test', async () => {
  const now = new Date().getTime()
  let i = 0
  const dummy = await createDummyAndAuthorize()
  do {
    i += 1
    await user.auth(`Bearer ${dummy.token!}`)
  } while (new Date().getTime() - now < 1000)
  // console.log(i)
})

describe('auth', () => {
  it('should resolve with true and valid userId for hardcoded token', async () => {
    const dummy = await createDummyAndAuthorize()
    await expect(user.auth(dummy.token)).resolves.toEqual({ userId: dummy.userId })
  })

  it('should resolve with false for invalid token', async () => {
    const response = await user.auth('invalidToken')
    expect(response).toEqual({ error: { type: 'unauthorized', message: 'Authentication Failed' } })
  })
})

describe('login', () => {
  it('should return JWT token, userId, expireAt to a valid login/password', async () => {
    const dummy = await createDummy()
    await expect(user.login(dummy.username, dummy.password)).resolves.toEqual({
      userId: dummy.userId,
      token: expect.stringMatching(/^[A-Za-z0-9-_=]+\.[A-Za-z0-9-_=]+\.?[A-Za-z0-9-_.+/=]*$/),
      expireAt: expect.any(Date)
    })
  })

  it('should reject with error if login does not exist', async () => {
    await expect(user.login(faker.internet.userName(), faker.internet.password())).resolves.toEqual({
      error: { type: 'non_existing_user', message: 'User does not exist' }
    })
  })

  it('should reject with error if password is wrong', async () => {
    const dummy = await createDummy()
    await expect(user.login(dummy.username, faker.internet.password())).resolves.toEqual({
      error: { type: 'invalid_password', message: 'Invalid Password' }
    })
  })
})

describe('createUser', () => {
  it('should resolve with true and valid userId', async () => {
    const password = faker.internet.password()
    const username = faker.internet.userName()

    await expect(user.createUser(username, password)).resolves.toEqual({
      userId: expect.stringMatching(/^[a-f0-9]{24}$/)
    })
  })

  it('should resolves with false & valid error if duplicate', async () => {
    const password = faker.internet.password()
    const username = faker.internet.userName()

    await user.createUser(username, password)

    await expect(user.createUser(username, password)).resolves.toEqual({
      error: {
        type: 'account_already_exists',
        message: `${username} already exists`
      }
    })
  })

  it('should reject if invalid input', async () => {
    const password = faker.internet.password()
    const username = '*&~12_'

    await expect(user.createUser(username, password)).rejects.toThrowError('validation failed')
  })
})
